...
def plot_multigene_expr_grayout(expr, vis, vis2, dim1, dim2, genes, file, figTitle, s=3):
    cmap = matplotlib.cm.Spectral_r
    fig, ax = plt.subplots(figsize=(7, 5))
    c = expr[list(map(lambda x: x.upper(), genes))].sum(axis = 1)
...
for i, title in zip(range(len(TC_SIG)), ['TC I', 'TC II', 'TC III', 'TC IV']):
    ...
    plot_multigene_expr_grayout(expr=imp_df.loc[ci, :], 
                                vis=umap_s.loc[ci, :], vis2=umap_s.loc[ci2, :], 
                                dim1='UMAP_1', dim2='UMAP_2', genes=TC_SIG[i],
                                file=file, figTitle=title)
    file.close()
    file = PdfPages('plots/gene-expr/UMAP-{}-combined-gene-expr-unimputed-{}.pdf'.format(cellgroup, title))
    plot_multigene_expr_grayout(expr=unimp_df.loc[ci, :], 
                                vis=umap_s.loc[ci, :], vis2=umap_s.loc[ci2, :], 
                                dim1='UMAP_1', dim2='UMAP_2', genes=TC_SIG[i],
                                file=file, figTitle=title)
    file.close()
