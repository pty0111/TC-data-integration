...
# lyu
sro2 <- readRDS("../GSE184175-lyu-2022/results/SRO.rds") %>% 
  subset(Clusters %in% c(15, 17, 20))
...
sro2 <- CreateSeuratObject(counts = counts, meta.data = sro2@meta.data) %>%
  ...
  ScaleData() %>% RunPCA(npcs = 50)
sro2$orig.ident <- "Lyu"
sro2$Cluster.prev <- paste0("Lyu C", sro2$Clusters)
...
dir.create("Seurat/results/integrated-with-Lyu")
write.csv(sro.combined@meta.data, file = "Seurat/results/integrated-with-Lyu/meta-data.csv")
write.csv(sro.combined@reductions$umap@cell.embeddings, file = "Seurat/results/integrated-with-Lyu/UMAP.csv")
saveRDS(sro.combined, file = "Seurat/results/integrated-with-Lyu/SRO.rds")
write.csv(sro.combined@assays$RNA@data, file = "Seurat/results/integrated-with-Lyu/unimputed-expr.csv")
saveRDS(as.matrix(sro.combined@assays$RNA@data), file = "Seurat/results/integrated-with-Lyu/unimputed-expr.rds")

sro.combined <- magic(sro.combined)
write.csv(sro.combined@assays$MAGIC_RNA@data, file = "Seurat/results/integrated-with-Lyu/imputed-expr.csv")
saveRDS(sro.combined@assays$MAGIC_RNA@data, file = "Seurat/results/integrated-with-Lyu/imputed-expr.rds")


# plots ####
sro.combined <- readRDS("Seurat/results/integrated-with-Lyu/SRO.rds")
dir.create("Seurat/plots/integrated-with-Lyu/QC", recursive=T)
pdf("Seurat/plots/integrated-with-Lyu/QC/cluster-QC.pdf", width = 15, height = 10)
Idents(sro.combined) <- sro.combined$Clusters
plot.all.QC(sro.combined, ident = "Clusters", col = pal.lyu$Clusters)
dev.off()
...
# UMAP plots #####
ggsave(
  filename = "Seurat/plots/integrated-with-Lyu/clusters.pdf", width = 15, height = 12,
  plot = plot.groups(sro.combined, col = pal.lyu$Clusters)
)

ggsave(
  filename = "Seurat/plots/integrated-with-Lyu/Sample.pdf", width = 15, height = 12,
  plot = plot.groups(
...
# annotations
pdf("Seurat/plots/integrated-with-Lyu/annotations.pdf", width = 15, height = 12)
# MTO
...
gp + geom_point(data = df[!lyu.idx, ], color = "grey", size = 1, alpha = 0.7) +
  ...
  geom_label_repel(data = cluster_labels, 
                   aes(x = UMAP_1, y= UMAP_2, label = Cluster.annot, color=Cluster.annot), 
                   size=5, show.legend = F) +
  scale_color_manual(values = pal.lyu$Cluster.annot) + 
  labs(color = "Lyu.annot")
dev.off()

# clusters
pdf("Seurat/plots/integrated-with-Lyu/clusters.pdf", width = 15, height = 12)
...
# Lyu
cluster_labels <- df[lyu.idx, ] %>% group_by(Cluster.prev) %>% dplyr::select(UMAP_1, UMAP_2) %>% summarize_all(mean)
gp + geom_point(data = df[!lyu.idx, ], color = "grey", size = 1, alpha = 0.7) +
  geom_point(aes(color = Cluster.prev), df[lyu.idx, ], size = 2, alpha = 0.9) +
  geom_label_repel(data = cluster_labels, 
                   aes(x = UMAP_1, y= UMAP_2, label = Cluster.prev, color=Cluster.prev), 
                   size=5, show.legend = F) +
  # scale_color_manual(values = pal$Clusters) + 
...
pdf("Seurat/plots/integrated-with-Lyu/cluster-QC-UMAP.pdf", width = 15, height = 12)
plot.groups(sro.combined, col = pal.lyu$Clusters)
...
# Breakdown bar ####
dir.create("Seurat/plots/integrated-with-Lyu/breakdown-barplots")
ggsave(
  ...
  filename = "Seurat/plots/integrated-with-Lyu/breakdown-barplots/Samples-in-Clusters.pdf", width = 10, height = 10
)